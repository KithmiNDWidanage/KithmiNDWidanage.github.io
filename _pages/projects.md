...
### Shared Control with Impedance Perception during Bilateral Robotic Teleoperation
*University of Sussex*

This work is focused on a teleoperated contact-rich robotic manipulation scenario at an unknown environment. In this ongoing research work, a shared control framework is designed for exerting a variable desired force on a surface during a continuous contact task. Utilising the NRP-ILC (see previous work) for environment stiffness estimation and autonomous force tracking, and utilising FVIC (see previous work) for maintaining safe contact, this framework will mitigate the issues of communication delay by introducing impedance perception as a feedback to the human operator and arbitrating the controller priority between the human and the robot controller based on the accuracy of the impedance perception. I am currently implementing this controller on a ROS 2-based teleoperation interface between the Kinova Gen3 7DOF robotic manipulator and the 3D Systems TouchX haptic device.
